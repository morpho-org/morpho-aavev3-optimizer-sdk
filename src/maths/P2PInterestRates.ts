...
import { Deltas, MarketSizeDelta } from "../types";
...
export default class P2PInterestRates {
  ...
  public computeP2PSupplyRatePerYear({
    ...
    if (poolSupplyRatePerYear.gt(poolBorrowRatePerYear))
      ...
      p2pSupplyRate = p2pRate.sub(
        this.__MATHS__.percentMul(
          p2pRate.sub(poolBorrowRatePerYear),
          reserveFactor
        )
      );
    }

    if (delta.scaledDelta.gt(0) && delta.scaledP2PTotal.gt(0)) {
      const proportionDelta = minBN(
        this.__MATHS__.indexDivUp(
          this.__MATHS__.indexMul(delta.scaledDelta, poolIndex),
          this.__MATHS__.indexMul(delta.scaledP2PTotal, p2pIndex)
        ),
      ...
      p2pSupplyRate = this.__MATHS__
        .indexMul(
          p2pSupplyRate,
          this.__MATHS__.INDEX_ONE.sub(proportionDelta).sub(proportionIdle)
        )
  ...
  public computeP2PBorrowRatePerYear({
    ...
    if (poolSupplyRatePerYear.gt(poolBorrowRatePerYear))
      ...
      p2pBorrowRate = p2pRate.add(
        this.__MATHS__.percentMul(
          poolBorrowRatePerYear.sub(p2pRate),
          reserveFactor
        )
      );
    }
    ...
    if (poolSupplyGrowthFactor.lte(poolBorrowGrowthFactor)) {
      ...
      p2pSupplyGrowthFactor = p2pGrowthFactor.sub(
        PercentMath.percentMul(
          p2pGrowthFactor.sub(poolSupplyGrowthFactor),
          reserveFactor
        )
      );
      ...
      p2pBorrowGrowthFactor = p2pGrowthFactor.add(
        PercentMath.percentMul(
          poolBorrowGrowthFactor.sub(p2pGrowthFactor),
          reserveFactor
        )
      );
    } else {
  ...
  private _computeP2PIndex(
    ...
    if (
      scaledP2PTotal.isZero() ||
      (scaledDelta.isZero() && proportionIdle.isZero())
    )
  ...
  private _weightedAverage(x: BigNumber, y: BigNumber, percentage: BigNumber) {
    // TODO: move to morpho-ethers-utils
