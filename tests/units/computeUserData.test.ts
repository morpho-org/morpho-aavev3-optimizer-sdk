...
describe("computeUserData", () => {
  ...
  it("totalBorrow is as expected", () => {
    ...
    const { totalBorrowOnPool, totalBorrowInP2P, totalBorrow } = adapter.computeUserData();
  ...
  it("totalSupply is as expected", () => {
    ...
    Object.entries(ADAPTER_MOCK.userMarketsData).forEach(([underlying, userMarketData]) => {
      ...
      expectedUserSupplyOnPool = expectedUserSupplyOnPool.add(
        userSupplyOnPool.mul(ADAPTER_MOCK.marketsData[underlying].chainUsdPrice).div(underlyingUnit)
      );
      ...
      expectedUserSupplyInP2P = expectedUserSupplyInP2P.add(
        userSupplyInP2P.mul(ADAPTER_MOCK.marketsData[underlying].chainUsdPrice).div(underlyingUnit)
      );
    });

    const expectedTotalSupply = expectedUserSupplyInP2P.add(expectedUserSupplyOnPool);
    const { totalSupplyOnPool, totalSupplyInP2P, totalSupply } = adapter.computeUserData();
